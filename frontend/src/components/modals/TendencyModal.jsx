    ...
    <div className="modal-backdrop overflow-y-auto" onClick={onClose}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', marginBottom: '0.25rem' }}>
              ...
              <span style={{ fontSize: '9px', color: '#6C5CE7', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em' }}>AI 독서 성향 분석</span>
            </div>
        ...
        {tendencyResult.qa_count === 0 ? (
          <p style={{ color: '#8F87B8', fontSize: '0.875rem', lineHeight: 1.7, textAlign: 'center', padding: '2rem 0' }}>{tendencyResult.tendency_summary}</p>
        ) : (
          <>
            <div style={{ padding: '1rem', background: 'rgba(108, 92, 231,0.05)', border: '1px solid rgba(108, 92, 231,0.12)', borderRadius: '0.875rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.625rem' }}>
                <div style={{ width: '2px', height: '0.875rem', background: 'linear-gradient(to bottom, #6C5CE7, #A78BFA)', borderRadius: '9999px' }} />
                <span style={{ fontSize: '9px', fontWeight: 900, color: '#5849C4', textTransform: 'uppercase', letterSpacing: '0.12em' }}>성향 요약</span>
              </div>
              <p style={{ fontSize: '0.875rem', lineHeight: 1.7, color: '#6E67A0' }}>{tendencyResult.tendency_summary}</p>
            </div>

            {tendencyResult.reading_lenses?.length > 0 && (
              <div>
                <p style={{ fontSize: '9px', fontWeight: 900, color: '#6C5CE7', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '0.625rem' }}>자주 쓰는 관점</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                  {tendencyResult.reading_lenses.map((lens, i) => (
                    <span key={i} style={{ fontSize: '0.8125rem', fontWeight: 700, color: '#6C5CE7', background: 'rgba(108, 92, 231,0.1)', border: '1px solid rgba(108, 92, 231,0.2)', padding: '4px 12px', borderRadius: '9999px' }}>{lens}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              {tendencyResult.strong_areas?.length > 0 && (
                <div style={{ padding: '0.875rem', background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.15)', borderRadius: '0.875rem' }}>
                  ...
                  {tendencyResult.strong_areas.map((a, i) => (
                    <p key={i} style={{ fontSize: '0.8125rem', color: '#6E67A0', lineHeight: 1.6, fontWeight: 500 }}>· {a}</p>
                  ))}
                </div>
              )}
            ...
            <p style={{ fontSize: '10px', color: '#C7C2E0', textAlign: 'right' }}>분석 기반: Q&A {tendencyResult.qa_count}개</p>
          </>
        )}
      </motion.div>
    </div>
