      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
            ...
            <h2 style={{ fontSize: '1.125rem', fontWeight: 900, color: '#2B1B0E' }}>독서 모임 개설</h2>
          </div>
          <button onClick={onClose} style={{ width: '2rem', height: '2rem', borderRadius: '9999px', background: 'rgba(139, 90, 43,0.08)', border: '1px solid rgba(139, 90, 43,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#8A7460' }}>
            <X size={14} />
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.875rem', alignItems: 'flex-start' }}>
            ...
            <div style={{ position: 'relative', flexShrink: 0 }}>
              <div
                ...
                style={{ width: '48px', height: '68px', borderRadius: '0.9rem', overflow: 'hidden', boxShadow: '2px 3px 0 rgba(139, 90, 43,0.2)', border: '2px solid rgba(139, 90, 43,0.18)', background: 'rgba(139, 90, 43,0.08)', cursor: 'pointer', position: 'relative' }}
              >
                {displayImage
                    ...
                    : (
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '0.375rem', marginBottom: '0.5rem' }}>
                        ...
                        <div
                          ...
                          title="기본 (아바타)"
                          style={{ aspectRatio: '1', borderRadius: '6px', overflow: 'hidden', cursor: 'pointer', background: 'linear-gradient(135deg,#8B5A2B,#D2914B)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: !form.room_image ? '2px solid #8B5A2B' : '2px solid transparent' }}
                        >
                        ...
                        {form.book_image && (
                          <div
                            ...
                            title="책 표지"
                            style={{ aspectRatio: '1', borderRadius: '6px', overflow: 'hidden', cursor: 'pointer', border: form.room_image === form.book_image ? '2px solid #8B5A2B' : '2px solid transparent' }}
                          >
                        ...
                        {/* 앨범 이미지 */}
                        {albumImages.map(img => (
                          <div key={img.id} style={{ position: 'relative', aspectRatio: '1', borderRadius: '6px', overflow: 'hidden', cursor: 'pointer', border: form.room_image === img.image_data ? '2px solid #8B5A2B' : '2px solid transparent' }}
            ...
            <div style={{ flex: 1 }}>
              ...
              <input
                ...
                style={{ color: '#2B1B0E', ...(attemptedSubmit && !form.title.trim() ? { borderColor: '#ef4444' } : {}) }}
              />
          ...
          <div>
            ...
            {form.book_title ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.625rem 0.875rem', background: 'rgba(139, 90, 43,0.06)', border: '1px solid rgba(139, 90, 43,0.2)', borderRadius: '0.875rem' }}>
                {form.book_image && <img src={form.book_image} alt="" style={{ width: '32px', height: '46px', objectFit: 'cover', borderRadius: '4px' }} />}
                <span style={{ flex: 1, fontSize: '0.875rem', fontWeight: 700, color: '#2B1B0E' }}>{form.book_title}</span>
              ...
              <div style={{ position: 'relative' }}>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <div style={{ position: 'relative', flex: 1 }}>
                    <Search style={{ position: 'absolute', left: '0.875rem', top: '50%', transform: 'translateY(-50%)', color: '#C4AD91' }} size={14} />
                    <input
                      className="form-input"
                      value={bookQuery}
                      ...
                      style={{ paddingLeft: '2.5rem', color: '#2B1B0E' }}
                    />
                  </div>
                  <button onClick={handleBookSearch} disabled={isSearching} style={{ padding: '0 1rem', background: 'rgba(139, 90, 43,0.08)', border: '1px solid rgba(139, 90, 43,0.2)', borderRadius: '0.875rem', fontSize: '0.8125rem', fontWeight: 800, color: '#8B5A2B', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
                    {isSearching ? <Loader2 size={14} className="animate-spin" /> : '검색'}
                  </button>
                </div>
                {bookResults.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, marginTop: '0.375rem', background: '#FBF6EC', border: '1px solid rgba(139, 90, 43,0.2)', borderRadius: '0.875rem', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', overflow: 'hidden' }}>
                    {bookResults.map((b, i) => (
                      <div key={i} onClick={() => selectBook(b)} style={{ display: 'flex', gap: '0.625rem', padding: '0.625rem 0.875rem', cursor: 'pointer', borderBottom: '1px solid rgba(139, 90, 43,0.06)', transition: 'background 0.15s' }} className="hover:bg-amber-50\/30">
                        {b.image && <img src={b.image} alt="" style={{ width: '28px', height: '40px', objectFit: 'cover', borderRadius: '3px', flexShrink: 0 }} />}
                        <div style={{ minWidth: 0 }}>
                          <p style={{ fontSize: '0.8125rem', fontWeight: 700, color: '#2B1B0E', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.title?.replace(/<\/?[^>]+>/g, '')}</p>
                          <p style={{ fontSize: '10px', color: '#8A7460', fontWeight: 600 }}>{b.author}</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {needsHostBook && (
            <div>
              ...
              <p style={{ fontSize: '10px', color: '#C4AD91', fontWeight: 600, marginBottom: '0.375rem' }}>
              ...
              {form.host_book_title ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.625rem 0.875rem', background: 'rgba(139, 90, 43,0.06)', border: '1px solid rgba(139, 90, 43,0.2)', borderRadius: '0.875rem' }}>
                  ...
                  <span style={{ flex: 1, fontSize: '0.875rem', fontWeight: 700, color: '#2B1B0E' }}>{form.host_book_title}</span>
                ...
                <div style={{ position: 'relative' }}>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    <button onClick={handleHostBookSearch} disabled={isSearchingHostBook} style={{ padding: '0 1rem', background: 'rgba(139, 90, 43,0.08)', border: '1px solid rgba(139, 90, 43,0.2)', borderRadius: '0.875rem', fontSize: '0.8125rem', fontWeight: 800, color: '#8B5A2B', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
                  ...
                  {hostBookResults.length > 0 && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, marginTop: '0.375rem', background: '#FBF6EC', border: '1px solid rgba(139, 90, 43,0.2)', borderRadius: '0.875rem', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', overflow: 'hidden' }}>
                      {hostBookResults.map((b, i) => (
                        <div key={i} onClick={() => selectHostBook(b)} style={{ display: 'flex', gap: '0.625rem', padding: '0.625rem 0.875rem', cursor: 'pointer', borderBottom: '1px solid rgba(139, 90, 43,0.06)' }}>
                          {b.image && <img src={b.image} alt="" style={{ width: '28px', height: '40px', objectFit: 'cover', borderRadius: '3px', flexShrink: 0 }} />}
                          <div style={{ minWidth: 0 }}>
                            <p style={{ fontSize: '0.8125rem', fontWeight: 700, color: '#2B1B0E', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.title?.replace(/<\/?[^>]+>/g, '')}</p>
                            <p style={{ fontSize: '10px', color: '#8A7460', fontWeight: 600 }}>{b.author}</p>
            ...
            {[
              ...
              <div key={key}>
                ...
                <div style={{ display: 'flex', gap: '0.375rem', marginBottom: '0.375rem' }}>
                  {TIME_OPTIONS.map(m => (
                    <button key={m} onClick={() => set(key, m)} style={{ flex: 1, padding: '0.4rem 0', borderRadius: '0.625rem', fontSize: '11px', fontWeight: 800, cursor: 'pointer', background: form[key] === m ? 'rgba(139, 90, 43,0.15)' : 'transparent', border: `1px solid ${form[key] === m ? 'rgba(139, 90, 43,0.35)' : 'rgba(139, 90, 43,0.15)'}`, color: form[key] === m ? '#8B5A2B' : '#8A7460', transition: 'all 0.15s', textAlign: 'center' }}>
                      {fmtMin(m)}
                    </button>
                  ))}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                  <input
                    ...
                    style={{ flex: 1, padding: '0.35rem 0.5rem', borderRadius: '0.625rem', border: '1px solid rgba(139, 90, 43,0.2)', background: 'rgba(139, 90, 43,0.04)', fontSize: '11px', fontWeight: 800, color: '#2B1B0E', outline: 'none', textAlign: 'center' }}
                  />
                  <span style={{ fontSize: '11px', color: '#8A7460', fontWeight: 700 }}>분</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <input
                  ...
                  style={{ flex: 1, accentColor: '#8B5A2B' }}
                />
                <span style={{ fontSize: '0.875rem', fontWeight: 900, color: '#2B1B0E', minWidth: '32px', textAlign: 'center' }}>{form.max_participants}명</span>
              </div>
            </div>
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <input
                  ...
                  className="form-input"
                  style={{ color: '#2B1B0E', textAlign: 'center' }}
                />
                <span style={{ fontSize: '0.8125rem', color: '#8A7460', fontWeight: 700, whiteSpace: 'nowrap' }}>분 전</span>
              </div>
              <p style={{ fontSize: '10px', color: '#C4AD91', fontWeight: 600, marginTop: '0.25rem' }}>0 = 제한 없음</p>
            </div>
          </div>

          {/* 채팅 허용 */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 1rem', background: 'rgba(139, 90, 43,0.04)', border: '1px solid rgba(139, 90, 43,0.12)', borderRadius: '0.875rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              {form.chat_enabled ? <MessageSquare size={14} style={{ color: '#8B5A2B' }} /> : <MessageSquareDashed size={14} style={{ color: '#C4AD91' }} />}
              <div>
                <p style={{ fontSize: '0.8125rem', fontWeight: 800, color: form.chat_enabled ? '#2B1B0E' : '#8A7460' }}>독서 중 채팅</p>
                <p style={{ fontSize: '10px', color: '#C4AD91', fontWeight: 600 }}>독서 시간 중 채팅 허용 여부 (토론 중엔 항상 열림)</p>
              </div>
            </div>
            <button
              onClick={() => set('chat_enabled', !form.chat_enabled)}
              style={{ padding: '0.35rem 1rem', borderRadius: '9999px', fontSize: '11px', fontWeight: 800, cursor: 'pointer', transition: 'all 0.2s', background: form.chat_enabled ? 'rgba(139, 90, 43,0.12)' : 'rgba(100,100,100,0.07)', border: `1px solid ${form.chat_enabled ? 'rgba(139, 90, 43,0.3)' : 'rgba(139, 90, 43,0.12)'}`, color: form.chat_enabled ? '#8B5A2B' : '#8A7460', flexShrink: 0 }}
            >
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              style={{ width: '100%', resize: 'none', color: '#2B1B0E', boxSizing: 'border-box' }}
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.625rem', marginTop: '0.5rem' }}>
            <button onClick={onClose} className="cute-pill-button" style={{ flex: 1, padding: '0.875rem', background: 'rgba(139, 90, 43,0.06)', fontSize: '0.875rem', fontWeight: 800, color: '#8A7460', cursor: 'pointer' }}>
              취소
            </button>
            <button
