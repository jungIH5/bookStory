  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(6px)' }}>
      ...
      >
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(43, 27, 14,0.97), rgba(43, 27, 14,0.97))', padding: '1.75rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ width: '56px', minWidth: '56px', height: '80px', borderRadius: '6px', overflow: 'hidden', background: 'rgba(139, 90, 43,0.2)', border: '1px solid rgba(139, 90, 43,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>
            {item.book_image
              ? <img src={item.book_image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <BookOpen size={20} style={{ color: '#8B5A2B' }} />}
          </div>
          <div style={{ minWidth: 0 }}>
            <p style={{ fontSize: '9px', fontWeight: 900, color: '#D2914B', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '0.375rem' }}>세션이 자동 종료됐어요</p>
            ...
            {item.book_title && <p style={{ fontSize: '12px', color: '#6E5A45', fontWeight: 700, marginTop: '0.25rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.book_title}</p>}
          </div>
        </div>
        ...
        <div style={{ padding: '1.25rem 1.75rem', background: 'rgba(139, 90, 43,0.04)', borderBottom: '1px solid rgba(139, 90, 43,0.1)', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
          ...
          <span style={{ fontSize: '0.875rem', fontWeight: 700, color: '#6E5A45' }}>자동 계산된 예상 시간</span>
          <span style={{ fontSize: '1rem', fontWeight: 900, color: '#2B1B0E', marginLeft: 'auto' }}>{formatReadingTime(item.estimated_seconds)}</span>
        </div>

        <div style={{ padding: '1.75rem' }}>
          <p style={{ fontSize: '1rem', fontWeight: 900, color: '#2B1B0E', marginBottom: '0.5rem' }}>독서기록을 켜둔 채로 자리를 비우셨나 봐요</p>
          <p style={{ fontSize: '0.8125rem', color: '#8A7460', fontWeight: 700, marginBottom: '1.25rem', lineHeight: 1.5 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '1.5rem' }}>
            <input
              ...
              style={{
                ...
                border: '1px solid rgba(139, 90, 43,0.25)', background: 'white',
                fontSize: '1rem', fontWeight: 800, color: '#2B1B0E', textAlign: 'center',
              }}
            />
            <span style={{ fontSize: '0.875rem', fontWeight: 700, color: '#6E5A45' }}>분</span>
          </div>
