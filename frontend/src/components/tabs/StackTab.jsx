...
export default function StackTab({
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      {viewMode === 'tower' ? (
        <div style={{ width: '100%', maxWidth: '460px', height: '70vh', maxHeight: '640px', minHeight: '380px', backgroundColor: '#2B1B0E', borderRadius: '3rem 3rem 2.5rem 2.5rem', border: '2px solid rgba(139, 90, 43,0.16)', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', boxShadow: '8px 10px 0 rgba(139, 90, 43,0.15), 0 32px 64px -16px rgba(0,0,0,0.6)', marginBottom: '2.5rem' }}>
            ...
            <div style={{ minHeight: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center', padding: '60px 48px 8px 48px' }}>
              {readBooks && readBooks.filter(b => b.status !== 'reading').length > 0 ? (
                      ...
                      >
                        ...
                        >
                          ...
                          {book.status === 'reading' && bookH >= 44 && (
                            <div style={{ position: 'absolute', top: '50%', right: book.image ? '88px' : '10px', transform: 'translateY(-50%)', background: 'rgba(210, 145, 75,0.28)', border: '1px solid rgba(210, 145, 75,0.55)', borderRadius: '9999px', padding: '2px 7px', fontSize: '9px', fontWeight: 900, color: '#D2914B', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>
                              읽는 중
                            </div>
                          )}
                          {/* 호버 툴팁 */}
                          <div className="hidden group-hover:flex" style={{ position: 'absolute', top: '-54px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(15,23,42,0.95)', backdropFilter: 'blur(8px)', border: '1px solid rgba(139, 90, 43,0.3)', padding: '7px 13px', borderRadius: '12px', fontSize: '11px', fontWeight: 900, zIndex: 300, whiteSpace: 'nowrap', boxShadow: '0 12px 32px rgba(0,0,0,0.7)', alignItems: 'center', gap: '7px' }}>
                            ...
                            <span style={{ color: 'rgba(210, 145, 75,0.8)', fontSize: '9px' }}>{pages}p</span>
                          </div>
                        </div>
                      </motion.div>
                    );
                ...
                <div style={{ marginBottom: '8rem', textAlign: 'center' }}>
                  <div className="cute-float" style={{ width: '68px', height: '68px', margin: '0 auto 1rem', borderRadius: '42% 58% 63% 37% / 47% 44% 56% 53%', background: 'rgba(139, 90, 43,0.1)', border: '2px solid rgba(139, 90, 43,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <BookOpen size={28} color="#D2914B" />
                  </div>
                  <p style={{ fontWeight: 900, fontSize: '1rem', color: '#8A7460', marginBottom: '0.375rem' }}>당신만의 지식 타워를 세우세요</p>
                  <p style={{ fontSize: '0.8125rem', color: '#C4AD91' }}>검색으로 책을 찾아 서재에 추가해보세요</p>
                </div>
              )}
            </div>
          </div>
          <div style={{ width: '100%', height: '24px', backgroundImage: 'linear-gradient(to bottom, #3D2712, #1F1408)', borderRadius: '0 0 2.5rem 2.5rem', borderTop: '2px solid rgba(139, 90, 43,0.2)', boxShadow: '0 -4px 12px rgba(0,0,0,0.4)', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
          {readBooks.filter(b => b.status !== 'reading').length === 0 && (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', borderRadius: '1.5rem', background: 'rgba(139, 90, 43,0.04)', border: '1px solid rgba(139, 90, 43,0.1)' }}>
              <BookOpen size={36} style={{ margin: '0 auto 1rem', color: '#C4AD91' }} />
              <p style={{ color: '#8A7460', fontWeight: 700 }}>완독한 책이 없습니다.</p>
            </div>
          )}
          {/* 완독 섹션 */}
          {readBooks.filter(b => b.status !== 'reading').length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.875rem' }}>
                <div style={{ width: '3px', height: '14px', borderRadius: '9999px', background: '#8B5A2B' }} />
                <span style={{ fontSize: '11px', fontWeight: 900, color: '#8B5A2B', textTransform: 'uppercase', letterSpacing: '0.1em' }}>완독</span>
                <span style={{ fontSize: '11px', fontWeight: 700, color: '#C4AD91' }}>{readBooks.filter(b => b.status !== 'reading').length}권</span>
              </div>
      ...
      {user && readBooks.length > 0 && (
        <div style={{ width: '100%', maxWidth: '460px', marginTop: '1.5rem', display: 'flex', gap: '0.75rem' }}>
          <button
            ...
            style={{ flex: 1, padding: '0.75rem', background: '#F0E4CC', border: '2.5px solid rgba(139, 90, 43,0.25)', color: '#6E4A1F', fontWeight: 800, fontSize: '0.8125rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
          >
          ...
          <button
            ...
            className="cute-pill-button"
            style={{ flex: 1, padding: '0.75rem', background: '#FBE8D6', border: '2.5px solid rgba(107, 66, 38,0.35)', color: '#7C4A21', fontWeight: 800, fontSize: '0.8125rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
          >
      ...
      {recommendations.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ width: '100%', marginTop: '2rem' }}>
          <div className="section-header" style={{ marginBottom: '1rem' }}>
            ...
            <h3 style={{ fontSize: '0.875rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#2B1B0E' }}>당신을 위한 추천 도서</h3>
          </div>
            ...
            {recommendations.map((book, i) => (
              <div key={i} onClick={() => onBookClick(book)} className="book-list-item group" style={{ cursor: 'pointer' }}>
                {book.image ? (
                  ...
                  <div style={{ width: '3rem', height: '4.25rem', borderRadius: '0.5rem', background: 'rgba(139, 90, 43,0.15)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><BookOpen size={18} style={{ color: '#8B5A2B' }} /></div>
                )}
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: '0.75rem', color: '#8A7460', fontWeight: 700, marginBottom: '0.375rem' }}>{book.author}</p>
                  <p style={{ fontSize: '11px', color: '#8B5A2B', lineHeight: 1.5 }}>{book.reason}</p>
                </div>
                <ChevronRight size={14} style={{ color: '#C4AD91', flexShrink: 0 }} />
              </div>
            ))}
          </div>
        </motion.div>
...
function BookListItem({ book, idx, onStackBookClick, onDeleteBook, onUpdatePages }) {
  ...
  return (
    ...
    >
      <div style={{ width: '3rem', minWidth: '3rem', height: '4.25rem', borderRadius: '0.5rem', overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,0.4)', flexShrink: 0, background: '#E8DCC5' }}>
        {book.image ? (
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '0.75rem', marginBottom: '0.25rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', flexShrink: 0 }}>
            <span style={{ fontSize: '9px', color: '#8B5A2B', fontWeight: 900, background: 'rgba(139, 90, 43,0.1)', border: '1px solid rgba(139, 90, 43,0.2)', padding: '2px 8px', borderRadius: '6px', letterSpacing: '0.05em' }}>
        ...
        <p style={{ fontSize: '0.75rem', color: '#8A7460', fontWeight: 700, marginBottom: '0.375rem' }}>{book.author}</p>
        {book.impression && (
          <p style={{ fontSize: '0.75rem', color: '#6E5A45', lineHeight: 1.5, marginBottom: '0.375rem', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            <span style={{ color: book.is_public ? '#8B5A2B' : '#C4AD91', fontWeight: 700, marginRight: '4px' }}>{book.is_public ? '🌐' : '🔒'}</span>
            {book.impression}
          </p>
        )}
        <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
          {editingPages ? (
            <input
              ...
              style={{ width: '64px', fontSize: '9px', padding: '2px 8px', borderRadius: '9999px', border: '1px solid rgba(139, 90, 43,0.4)', background: 'white', color: '#4A2F17', outline: 'none' }}
            />
          ) : (
            <span
              onClick={onUpdatePages ? handlePagesClick : undefined}
              title={onUpdatePages ? '클릭해서 페이지 수 수정' : undefined}
              style={{ fontSize: '9px', color: '#8A7460', border: '1px solid rgba(139, 90, 43,0.12)', padding: '2px 8px', borderRadius: '9999px', background: 'rgba(139, 90, 43,0.04)', cursor: onUpdatePages ? 'text' : 'default' }}
            >{book.pages || 250}p</span>
          )}
          {book.publisher && <span style={{ fontSize: '9px', color: '#8A7460', border: '1px solid rgba(139, 90, 43,0.12)', padding: '2px 8px', borderRadius: '9999px', background: 'rgba(139, 90, 43,0.04)' }}>{book.publisher}</span>}
          {!book.impression && <span onClick={e => { e.stopPropagation(); onStackBookClick(book); }} style={{ fontSize: '9px', color: '#8B5A2B', border: '1px solid rgba(139, 90, 43,0.2)', padding: '2px 8px', borderRadius: '9999px', background: 'rgba(139, 90, 43,0.05)', cursor: 'pointer' }}>+ 감상평 쓰기</span>}
        </div>
      </div>
    </motion.div>
  );
