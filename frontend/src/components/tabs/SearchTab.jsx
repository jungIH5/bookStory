    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ width: '100%', marginTop: '0.5rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, 160px)', gap: '1.75rem 1rem', width: '100%', justifyContent: 'center' }}>
        {searchResults.map((book, index) => (
          ...
          >
            <div style={{ width: '100%', aspectRatio: '3/4', marginBottom: '0.875rem', overflow: 'hidden', borderRadius: '1.25rem', position: 'relative', boxShadow: '0 16px 32px rgba(0,0,0,0.4)', border: '2px solid rgba(139, 90, 43,0.14)', background: '#E8DCC5' }}>
              {book.image ? (
                <img
                  src={book.image}
              ...
              <div className="group-hover:opacity-100 opacity-0 transition-opacity duration-300" style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(43, 27, 14,0.92) 0%, rgba(139, 90, 43,0.5) 55%, transparent 80%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: '0.75rem' }}>
                <span style={{ fontSize: '10px', fontWeight: 900, color: 'white', letterSpacing: '0.12em', textTransform: 'uppercase' }}>자세히 보기</span>
                <div style={{ width: '20px', height: '1.5px', background: 'rgba(210, 145, 75,0.9)', marginTop: '4px', borderRadius: '9999px' }} />
              </div>
            </div>
            <div style={{ width: '100%', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '11px', color: '#8A7460', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', padding: '0 0.5rem' }}>{book.author}</p>
      ...
      {searchResults.length > 0 && (
        <div ref={sentinelRef} style={{ display: 'flex', justifyContent: 'center', padding: '2rem 0' }}>
          {isFetchingMore && <Loader2 size={20} className="animate-spin" style={{ color: '#8B5A2B' }} />}
        </div>
      )}
      {searchResults.length === 0 && searchQuery && !isSearching && (
        <div className="blob-card" style={{ textAlign: 'center', padding: '5rem 2rem', marginTop: '1rem' }}>
          <span style={{ fontSize: '2.5rem', display: 'block', marginBottom: '0.5rem' }}>🔍</span>
          <p style={{ color: '#8A7460', fontWeight: 700 }}>검색 결과가 없습니다.</p>
          <p style={{ color: '#C4AD91', fontSize: '0.8125rem', marginTop: '0.375rem' }}>다른 키워드로 검색해보세요.</p>
        </div>
      )}
      {searchResults.length === 0 && !searchQuery && !isSearching && (
        <div style={{ textAlign: 'center', padding: '5rem 2rem' }}>
          <Search size={48} style={{ margin: '0 auto 1rem', color: '#8A7460' }} />
          <p style={{ color: '#C4AD91', fontWeight: 700 }}>읽고 싶은 책을 검색해보세요</p>
        </div>
      )}
    </motion.div>
  );
